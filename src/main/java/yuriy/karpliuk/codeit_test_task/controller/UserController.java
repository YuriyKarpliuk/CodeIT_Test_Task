...
public class UserController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> updateAllUserFields(@PathVariable Long id, @Valid @RequestBody UserRequest userRequest) throws NotFoundException {
        return userService.updateAllUserFields(id, userRequest);
    }

    @PatchMapping("/{id}")
    public ResponseEntity<?> updateUserFields(@PathVariable Long id, @Valid @RequestBody Map<String, Object> fields) throws NotFoundException {
    ...
    public ResponseEntity<?> getUsersByBirthDateRange(@RequestParam("start") LocalDate startDate,
                                                      @RequestParam("end") LocalDate endDate) throws NotFoundException, NotValidDateRange {
